...
EXT_PROHIBITED_SYMBOLS = frozenset(['/'])
WIN_PROHIBITED_SYMBOLS = frozenset(['/', '\\', ':', '*', '?', '"', '<', '>', '|'])
WIN_PROHIBITED_NAMES = frozenset(
    [
        'CON',
        'PRN',
        'AUX',
...
def check_length_limit(
    context: FileContext, limit: int, filesystems: str, encode: bool = True, check_path: bool = True
) -> str | None:
...
def rule_ext_filename_limit(context: FileContext, filesystems: str) -> str | None:
    return check_length_limit(
        context=context, limit=EXT_FILENAME_LENGTH_LIMIT_BYTES, filesystems=filesystems, check_path=False
    )


def rule_ext_encrypted_filename_limit(context: FileContext, filesystems: str) -> str | None:
    return check_length_limit(
        context=context, limit=EXT_ENCRYPTED_FILENAME_LENGTH_LIMIT_BYTES, filesystems=filesystems, check_path=False
    )
...
def rule_win_filename_limit(context: FileContext, filesystems: str) -> str | None:
    return check_length_limit(
        context=context,
        limit=WIN_FILENAME_LENGTH_LIMIT_SYMBOLS,
...
def rule_ntfs_path_length_limit(context: FileContext, filesystems: str) -> str | None:
    return check_length_limit(
        context=context, limit=NTFS_FULL_PATH_LIMIT_SYMBOLS, filesystems=filesystems, encode=False
    )


def rule_exfat_path_length_limit(context: FileContext, filesystems: str) -> str | None:
    return check_length_limit(
        context=context, limit=EXFAT_FULL_PATH_LIMIT_SYMBOLS, filesystems=filesystems, encode=False
    )


def rule_symbols_not_allowed(context: FileContext, prohibited_symbols: frozenset[str], filesystems: str) -> str | None:
    issue = None
    prohibited_symbols_found = ''.join(frozenset(context.filename) & prohibited_symbols)
    if prohibited_symbols_found:
        issue = f'{context.full_path} contains "{prohibited_symbols_found}", which isn\'t allowed on {filesystems}'
    return issue
...
def rule_case_insensitive(context: FileContext, filesystems: str) -> str | None:
    ...
    if any(sibling.lower() == filename_lower for sibling in context.siblings):
        issue = (
            f'{context.full_path} has case-insensitive duplicate filenames in the same directory which '
            f"isn't allowed on {filesystems}"
        )
    return issue
...
class Rule:
    ...
    def filesystems_displayed(self) -> str:
        if self.filesystems_enabled is None:
            raise RuntimeError(
                'Cannot access filesystems_displayed before rules have been enabled by get_rules_for_filesystems.'
            )
...
def main() -> int:
    ...
    parser.add_argument(
        '-f',
        '--filesystems',
        help='One or more filesystems to check compatibility with (choices: %(choices)s, all of them if not specified)',
        choices=FILESYSTEMS_SUPPORTED,
        default=FILESYSTEMS_SUPPORTED,
        nargs='*',
        metavar='FS',
    )
    ...
    print_issues(
        issues=issues, dir_count=dir_count, file_count=file_count, directory=directory, filesystems=filesystems
    )
    return 0
